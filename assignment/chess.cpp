 ...
 bool is_piece(struct ChessPiece board ,enum PieceColor color, enum PieceType type)
{
 ...
 struct ChessPiece get_piece(ChessBoard chess_board,char a, int one)
 {
   if(((a > 'h' || a < 'a') || (one > 8 || one < 1)) || chess_board[one-1][a -97].is_occupied == false)
   {
     chess_board[one-1][a - 97].piece.type = NoPiece;
     return chess_board[one-1][a - 97].piece;
   }
   return chess_board[one-1][a-97].piece;
}
